...
async function checkIfParamsExist(envName) {
  try {
    ...
    console.log('firstParameterPath:', firstParameterPath);
  ...
  console.log('Error checking parameters: Parameter not found:');
  return null;
}
}
...
async function getOtherEnvParams(copyFromEnv) {
  try {
      ...
      for (const parameter of parameters) {
          ...
          try {
            const command = new GetParameterCommand({ Name: parameterPath, WithDecryption: true });
      ...
      console.log('Error retrieving parameters: Parameter not found');
      throw error;
  }
}
  ...
  try {
   ...
   const envName = parsedParams.envName;
    ...
    if (!paramsExist) {
      ...
      await getOtherEnvParams(copyFromEnv); 
